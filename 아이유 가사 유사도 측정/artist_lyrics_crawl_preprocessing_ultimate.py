...
html = urlopen(
    "https://music.naver.com/artist/track.nhn?artistId=%s&filteringOptions=RELEASE&sorting=newRelease" % artist_id)

# 1페이지가 최대인경우 a태그가 존재하지 않으므로 수동으로 설정해야함
max_page = len(BeautifulSoup(html, "lxml").find(
    'div', class_='paginate2').find_all('a'))
if(max_page == 0):
    max_page = 1

for page in range(1, max_page+1, 1):
    html = urlopen(
        "https://music.naver.com/artist/track.nhn?artistId=%s&filteringOptions=RELEASE&sorting=newRelease&page=%d" % (artist_id, page))

    tr_list = BeautifulSoup(html, "lxml").find_all(
        'tr', class_='_tracklist_move')
    ...
    for tr in tr_list:
        ...
        song = tr.find('td', class_='tb_name').find(
            'span', class_='tit').text.strip('\n')
...
for t in trackId_list:
    ...
    # remove blank
    lyrics_sentence_list = list(
        filter(lambda a: a != '', lyrics_sentence_list))
    lyrics_sentence_list = list(
        filter(lambda a: a.isspace() != True, lyrics_sentence_list))
    lyrics_sentence_list = [re.sub(' +', ' ', ly)
...
df_track_info = pd.DataFrame.from_dict(
    trackInfoSheet_column_list, orient='index')
df_track_info = df_track_info.transpose()

df_sentence_data = pd.DataFrame.from_dict(
    SentenceSheet_column_list, orient='index')
df_sentence_data = df_sentence_data.transpose()

writer = pd.ExcelWriter("%s.xlsx" % artist_name)

df_track_info.to_excel(writer, sheet_name='track_info',
                       startrow=1, header=True)
df_sentence_data.to_excel(
    writer, sheet_name='Sentence_data', startrow=1, header=True)
